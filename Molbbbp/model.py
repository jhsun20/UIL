...
class CausalGraphon(torch.nn.Module):
    ...
    def forward(self, batch, epoch=0, eval_random=True):

        x, edge_index, edge_attr, batch_idx, label = batch.x, batch.edge_index, batch.edge_attr, batch.batch, batch.y
        x_encode = self.graph_front(x, edge_index, edge_attr, batch)
        causaler_output = self.causaler(batch)
        ...
        h_node_cau = self.graph_backs(x_encode, edge_index, edge_attr, batch_idx, node_cau, edge_cau)
        h_node_env = self.graph_backs(x_encode, edge_index, edge_attr, batch_idx, node_env, edge_env)
        h_graph_cau = self.pool(h_node_cau, batch_idx)
        h_graph_env = self.pool(h_node_env, batch_idx)
        ...
        if self.args.graphon and epoch >= self.args.graphon_pretrain and epoch % self.args.graphon_frequency == 0:
            graphons, ys, envs = estimate_graphon(batch, edge_cau.squeeze(), self.N, h_graph_env, self.args.num_env)
            if graphons == None:
                return output
            intra_y = []
